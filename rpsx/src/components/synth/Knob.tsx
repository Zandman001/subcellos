...
export default function Knob({ label, value, onChange, step, format, disabled, dragScale, inactive, infinite, onStepClick }: KnobProps) {
  ...
  const ANG_MIN = -135;
  const ANG_MAX = 135;
  // No orbit animation anymore; replaced by static 1-bit segment ring.
  ...
  const angle = ANG_MIN + v * (ANG_MAX - ANG_MIN); // retained for potential future pointer-less math
  const ticks = 16; // fewer, chunkier segments for Ark style
  const activeTicks = Math.round(v * (ticks));
  ...
  <div className={`ctrl${inactive ? ' inactive' : ''} ark-knob`} style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: 3, width: 60, opacity: disabled ? 0.5 : 1 }}>
      ...
      >
        <div className="knob-face ark-flat">
          <div className="ark-seg-ring">
