...
export default function SynthOSC() {
  ...
  const setShape = (v:number)=> { const nv = clamp01(v); updateOsc(s, idx, { shape: nv }); s.setSynthParam(`part/0/${idx}/shape`, Math.round(nv*7), 'I32'); };
  const setDetune = (v:number)=> { const nv = clamp01(v); updateOsc(s, idx, { detune: nv }); s.setSynthParam(`part/0/${idx}/detune_cents`, (nv-0.5)*100, 'F32'); };
  const setFm = (v:number)=> { const nv = clamp01(v); updateOsc(s, idx, { fm: nv }); const fmPath = idx === 'oscA' ? `part/0/oscA/fm_to_B` : `part/0/oscB/fm_to_A`; s.setSynthParam(fmPath, nv); };
  const setLevel = (v:number)=> { const nv = clamp01(v); updateOsc(s, idx, { level: nv }); s.setSynthParam(`part/0/${idx}/level`, nv); };
  useFourKnobHotkeys({
  ...
  <Knob label="Detune" value={osc.detune} step={49} onChange={(v)=> { updateOsc(s, idx, { detune: v }); s.setSynthParam(`part/0/${idx}/detune_cents`, (v-0.5)*100, 'F32'); }} format={(v)=>`${Math.round((v-0.5)*100)}c`} />
  <Knob label="FM Amt" value={osc.fm} step={49} onChange={(v)=> { updateOsc(s, idx, { fm: v }); const fmPath = idx === 'oscA' ? `part/0/oscA/fm_to_B` : `part/0/oscB/fm_to_A`; s.setSynthParam(fmPath, v); }} format={(v)=>`${Math.round(v*100)}%`} />
  <Knob label="Level" value={osc.level} step={49} onChange={(v)=> { updateOsc(s, idx, { level: v }); s.setSynthParam(`part/0/${idx}/level`, v); }} format={(v)=>`${Math.round(v*100)}%`} />
      </Row>
    </Page>
  );
}
