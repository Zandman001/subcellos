  ...
  return (
    <Page title={`LFO`}>
      ...
      <Row>
        ...
        <Knob label="Rate" value={lfo.rate} onChange={(v)=> { updateLfo(s, { rate: v }); s.setSynthParam(`part/0/lfo/rate_hz`, mapRate(v)); }} format={(v)=>`${mapRate(v).toFixed(2)} Hz`} />
        <Knob label="Amount" value={lfo.amount ?? 1} onChange={(v)=> { updateLfo(s, { amount: v }); s.setSynthParam(`part/0/lfo/amount`, v); }} format={(v)=>`${Math.round(v*100)}%`} />
        <Knob label="Drive" value={lfo.drive} onChange={(v)=> { updateLfo(s, { drive: v }); s.setSynthParam(`part/0/lfo/drive`, v); }} format={(v)=>`${Math.round(v*100)}%`} />
      </Row>
    </Page>
  );
}
