...
impl DrumPlayer {
    ...
    pub fn render(&mut self, params: &ParamStore, keys: &DrumParamKeys) -> DrumRenderFrame {
        ...
        for voice in &mut self.voices {
            ...
            let slot = voice.slot;
            let sample = match self.samples.get(slot) {
                Some(s) if s.len > 0 => s,
                _ => {
                    voice.active = false;
                    continue;
            ...
            let volume = params
                .get_f32_h(keys.slot_volume[slot], 0.85)
                .clamp(0.0, 1.5);
            ...
            match decoder.decode(&packet).map_err(|e| e.to_string())? {
                ...
                AudioBufferRef::S24(buf) => {
                    ...
                    if ch == 1 {
                        for &s in b.chan(0) {
                            let si = s.inner();
                            data.push(si as f32 / 8_388_608.0);
                        }
                    } else if ch >= 2 {
                        let left = b.chan(0);
                        let right = b.chan(1);
                        for (&l, &r) in left.iter().zip(right.iter()) {
                            ...
                            let ri = r.inner();
                            data.push(
                                ((li as f32) / 8_388_608.0 + (ri as f32) / 8_388_608.0) * 0.5,
                            );
                        }
                    }
                }
