        ...
        match exciter_type {
            ...
            2 => {
                ...
                if self.impulse_counter > 0 {
                    ...
                    let click_sample = if self.impulse_counter % 2 == 0 {
                        1.0
                    } else {
                        -1.0
                    };
...
impl ResonatorVoice {
    ...
    pub fn render(&mut self, params: &ParamStore, param_keys: &ResonatorParamKeys) -> f32 {
        ...
        if params_changed || self.just_triggered {
            ...
            match mode {
                0 => {
                    // Modal mode - harmonic resonators (Brightness = tonal tilt; dark adds undertones)
                    // Tone shaping
                ...
                1 => {
                    // Comb mode - tone shaping and optional undertone; no brightness->pitch coupling
                    if bank_size > 0 {
                        let tone = brightness.clamp(0.0, 1.0);
                        ...
                        let q_max = 12.0;
                        let q =
                            q_min + (q_max - q_min) * decay_e * (1.0 + 0.2 * bright - 0.15 * dark);
                        ...
                        if dark > 0.05 && bank_size >= 2 {
                            ...
                            self.resonators[1].set_bandpass(
                                sub_f.min(self.sr * 0.45),
                                sub_q,
                                self.sr,
                            );
                ...
                _ => {
                    ...
                    for i in 0..bank_size {
                        ...
                        self.resonators[i].set_bandpass(
                            harmonic_freq.min(self.sr * 0.45),
                            q,
                            self.sr,
                        );
        ...
        if mode == 1 && bank_size > 0 {
            // Comb mode with feedback
            // Scale feedback to be more audible (0-1 UI range to 0-0.98 for stability)
            let scaled_feedback = feedback * 0.98;
            let resonator_out =
                self.resonators[0].process(driven_excitation + output * scaled_feedback);
            ...
            // Modal mode - feedback adds inter-resonator coupling
