...
impl Smooth {
    ...
    #[inline]
    #[allow(dead_code)]
    pub fn set_tau(&mut self, sr: f32, ms: f32) {
        self.a = (-1.0 / (ms * 0.001 * sr)).exp();
    }
    #[inline]
    pub fn next(&mut self, target: f32) -> f32 {
        self.y = self.a * self.y + (1.0 - self.a) * target;
        self.y
    }
}
...
impl SimpleDelay {
    ...
    pub fn process_block(
        ...
        for n in 0..l.len() {
            ...
            // advance
            self.wr_l += 1;
            if self.wr_l >= self.len_l {
                self.wr_l = 0;
            }
            self.wr_r += 1;
            if self.wr_r >= self.len_r {
                self.wr_r = 0;
            }
        }
    }
