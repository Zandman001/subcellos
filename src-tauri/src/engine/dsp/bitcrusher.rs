...
impl Bitcrusher {
    ...
    pub fn set_bits(&mut self, b: u8) {
        self.bits = b.max(4).min(16);
    }
    pub fn set_factor(&mut self, f: u32) {
        self.factor = f.max(1).min(16);
    }
    ...
    pub fn process(&mut self, l: &mut [f32], r: &mut [f32]) {
        if self.mix <= 0.0001 || (self.bits >= 16 && self.factor <= 1) {
            return;
        }
        let mix = self.mix;
        let dry = 1.0 - mix;
