use std::{collections::HashMap, fs, io::Write, path::{Path, PathBuf}, thread, time::Duration};
...
fn sounds_dir(project: &str) -> Result<PathBuf, String> { Ok(project_dir(project)?.join("sounds")) }
fn sound_preset_file(project: &str, sound_id: &str) -> Result<PathBuf, String> { Ok(sounds_dir(project)?.join(format!("{}.json", sound_id))) }

// Debounced atomic writer per-path
static WRITE_WORKERS: OnceCell<std::sync::Mutex<HashMap<PathBuf, Sender<Vec<u8>>>>> = OnceCell::new();

fn schedule_write(path: PathBuf, data: Vec<u8>) -> Result<(), String> {
  let map = WRITE_WORKERS.get_or_init(|| std::sync::Mutex::new(HashMap::new()));
  let mut map = map.lock().map_err(|_| "write workers poisoned".to_string())?;
  let tx = if let Some(tx) = map.get(&path) { tx.clone() } else {
...
fn atomic_write(path: &Path, data: &[u8]) -> Result<(), String> {
  if let Some(dir) = path.parent() { fs::create_dir_all(dir).map_err(|e| format!("create dir: {e}"))?; }
...
pub fn fs_create_project() -> Result<String, String> {
  ...
  let name = loop {
    ...
    if !root.join(&candidate).exists() { break candidate; }
    n += 1;
  };
...
pub fn fs_delete_project(name: String) -> Result<(), String> {
  ...
  if dir.exists() { fs::remove_dir_all(dir).map_err(|e| format!("rm project: {e}"))?; }
  Ok(())
}
...
pub fn fs_delete_pattern(project: String, pattern: String) -> Result<(), String> {
  ...
  if dir.exists() { fs::remove_dir_all(dir).map_err(|e| format!("rm pattern: {e}"))?; }
  Ok(())
}
...
pub fn fs_read_project(project: String) -> Result<Project, String> {
  ...
  if !file.exists() { return Ok(Project::default()); }
...
pub fn fs_read_pattern(project: String, pattern: String) -> Result<Pattern, String> {
  ...
  if !file.exists() { return Ok(Pattern::default()); }
...
pub fn delete_sound(project_name: String, sound_id: String) -> Result<(), String> {
  ...
  let mut pj: Project = if pfile.exists() { read_json(&pfile)? } else { Project::default() };
...
pub fn create_sound(project_name: String, sound_type: String) -> Result<Sound, String> {
  ...
  used.sort_unstable();
  let mut pi: usize = 0;
  for u in used { if u == pi { pi += 1; } }
  if pi >= 16 { return Err("no free part index".to_string()); }

  let sound = Sound { id: new_id(), kind, name: display_name, part_index: pi };
...
pub fn load_sound_preset(project: String, sound_id: String) -> Result<String, String> {
  ...
  if !file.exists() { return Err("not_found".to_string()); }
